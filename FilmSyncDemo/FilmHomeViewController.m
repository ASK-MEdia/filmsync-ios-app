...
- (IBAction)testMarkerButtonPressed:(id)sender
{
    ...
    [self newMarkerReceived:self.testMarkerTextField.text];
}
...
-(void)getMarkerDict
{
    ...
    */
    ...
    NSUserDefaults *defaults =  [NSUserDefaults standardUserDefaults];
    NSString *status = [defaults stringForKey:@"kFilmSyncAuthStatus"];
    NSLog(@"status :%@",status);
    
    if ([status isEqualToString:@"valid"])
    {
        NSURL *URL = [NSURL URLWithString:@"http://10.10.2.90/filmsync/getAllCardsForProject.json"];
...
-(void)AuthenticateWithServer
{
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request
                                  ...
                                  ^(NSData *data, NSURLResponse *response, NSError *error) {
                                      ...
                                      if (!error)
                                      {
                                          NSString *Status = [jsonDict objectForKey:@"expired"];
                                          
                                          NSUserDefaults *defaults =  [NSUserDefaults standardUserDefaults];
                                          [defaults setObject:Status forKey:@"kFilmSyncAuthStatus"];
